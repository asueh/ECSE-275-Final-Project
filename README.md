...
The manipulation layer of the robot, implemented in Lua, acts as the bridge between the perception system and the physical simulation. This component was responsible for resolving the non-linear relationship between the desired end-effector position (Task Space) and the required joint angles (Joint Space)1.Implementation and ECSE 275 ConceptsI programmed the Inverse Kinematics (IK) solver using the Damped Least Squares method and architected the Lua-based Finite State Machine (FSM) to control the robot's complete lifecycle.Inverse Kinematics (IK) Solver:Concept: Applied the numerical iterative approach using the Damped Least Squares (DLS) method to solve the IK problem2.Technical Challenge: The initial target standoff of $1.87 \text{ m}$ (from the initial path planning concept) placed the apple far outside the arm's functional workspace ($<1.2 \text{ m}$ reach).Resolution: After extensive testing, the mobile base was forced to stop at a maximum distance of $0.3 \text{ m}$ to $0.6 \text{ m}$ from the target apple to ensure kinematic feasibility. 2.  IK Constraint and Singularity Management:Problem: The arm repeatedly exhibited twisting and joint freezing (Joints 3 and 4 failure) due to the IK solver operating near kinematic singularities or struggling to meet the fixed end-effector orientation constraint3.Resolution (Constraint Relaxation): The compute_smart_path_world function was modified to use dynamic constraints. During the picking and lifting phases, the IK solver was instructed to prioritize only the position ($\text{XYZ}$) of the tip (simIK.constraint_position), allowing the rotation to float. This successfully reduced twisting and maximized reach by relieving the strain on the solver.Trajectory Generation (Snake Path):Goal: To prevent the "elephant drag" effect and avoid obstacles (like the treetop), a smooth, efficient trajectory was needed.Method: The execute_move function utilizes a 3rd-order polynomial interpolation in joint space between the solved IK waypoints. This method inherently favors the shortest path in the configuration space, creating the desired smooth, snake-like movement that avoids large, sweeping movements in the task space4444.Combining Components (The Integration Challenge):Active Pose Holding (The Physics Fix): A major hurdle was preventing the high inertial forces of the moving arm from causing the mobile base to shift, especially given the strict physics simulation.Solution: I developed an Active Pose Holding system. Upon stopping, the script captures the chassis's position and orientation ($\text{X, Y, Z, Roll, Pitch, Yaw}$) and forces the base to reset to these coordinates every simulation step during the entire kinematics sequence. This created a perfectly stable platform without triggering the "link separation" physics errors associated with switching the dynamic chassis to static mode.Orientation Alignment: The initial drive logic was simplified to a pure straight-line drive (turning factor $\text{turn}=0.0$), eliminating unwanted steering that caused the $180^\circ$ rotation issues and simplifying the required alignment check.
